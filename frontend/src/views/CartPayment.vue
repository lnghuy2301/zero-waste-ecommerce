    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
      <div class="lg:col-span-5">
        <div class="bg-white/80 rounded-[2rem] p-6 shadow-xl border-2 border-white/50">
          <div class="flex items-center justify-between mb-8 pb-4 border-b-2 border-dashed border-slate-100">
            ...
            <button @click="removeSelectedItems" class="text-red-500 font-black text-[10px] uppercase hover:underline" :disabled="selectedIds.length === 0">
              XÓA ĐÃ CHỌN
            </button>
          </div>

          <div class="space-y-6 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
            <div v-for="item in cartItems" :key="item.id" class="flex gap-4 items-center bg-white p-3 rounded-2xl border border-slate-50 shadow-sm">
              <input type="checkbox" :value="item.id" v-model="selectedIds" class="w-4 h-4 accent-[#658a22]" />

              <div class="w-20 h-20 rounded-xl bg-slate-50 border border-slate-100 shrink-0 overflow-hidden">
                <img :src="getImageUrl(item.productImage || item.variant?.product?.mainImage || item.product?.mainImage)" class="w-full h-full object-cover" />
              </div>

              <div class="flex-1 min-w-0">
                <h4 class="font-bold text-slate-900 text-[13px] leading-snug line-clamp-2 mb-1">
                  {{ item.productName || item.name || item.variant?.product?.name || item.product?.name || 'Sản phẩm' }}
                </h4>
                <p v-if="item.variantName || item.variant?.name" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">
                ...
                <div class="flex justify-between items-center mt-2">
                  <div class="flex items-center bg-slate-100 rounded-lg overflow-hidden border border-slate-200">
                    <button @click="updateQuantity(item, -1)" class="px-2 py-1 text-slate-500 font-black hover:bg-slate-200">-</button>
                    <span class="px-2 py-1 text-[11px] font-black">{{ item.quantity }}</span>
                    <button @click="updateQuantity(item, 1)" class="px-2 py-1 text-slate-500 font-black hover:bg-slate-200">+</button>
                  </div>
                  <div class="flex items-center gap-3">
                    <span class="font-black text-[#658a22] text-sm">{{ formatVND(item.price || item.variant?.price || 0) }}</span>
                    <button @click="removeItem(item.id)" class="text-slate-300 hover:text-red-500"><span class="material-symbols-outlined text-[18px]">delete</span></button>
                  </div>
                </div>
              </div>
            </div>

            <div v-if="cartItems.length === 0" class="text-center py-10 text-slate-400 font-bold uppercase text-[10px]">
              Giỏ hàng đang trống
            </div>
          </div>

          <div class="mt-8 pt-6 border-t-2 border-slate-100 space-y-3 font-bold text-[11px] uppercase text-slate-400">
            <div class="flex justify-between"><span>Tạm tính</span><span class="text-slate-600">{{ formatVND(subtotal) }}</span></div>
            <div class="flex justify-between"><span>Phí ship</span><span class="text-slate-600">{{ shippingFee === 0 ? 'Miễn phí' : formatVND(shippingFee) }}</span></div>
            <div class="flex justify-between text-slate-900 text-xl pt-4 border-t-2 border-dashed italic">
              <span>Tổng</span><span class="text-[#658a22] font-black">{{ formatVND(total) }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-7 space-y-8">
        <section class="bg-white/80 p-8 rounded-[2rem] shadow-xl border-2 border-white/50">
          ...
          <div class="grid grid-cols-2 gap-4">
            <input v-model="shippingInfo.fullName" placeholder="Họ tên người nhận" class="col-span-1 p-4 bg-white/50 border-2 border-slate-100 rounded-2xl focus:border-[#658a22] outline-none font-bold text-sm" />
            <input v-model="shippingInfo.phone" placeholder="Số điện thoại" class="col-span-1 p-4 bg-white/50 border-2 border-slate-100 rounded-2xl focus:border-[#658a22] outline-none font-bold text-sm" />
            <input v-model="shippingInfo.address" placeholder="Địa chỉ chi tiết (Số nhà, đường...)" class="col-span-2 p-4 bg-white/50 border-2 border-slate-100 rounded-2xl focus:border-[#658a22] outline-none font-bold text-sm" />
            <textarea v-model="shippingInfo.note" placeholder="Ghi chú thêm cho shipper..." class="col-span-2 p-4 bg-white/50 border-2 border-slate-100 rounded-2xl focus:border-[#658a22] outline-none font-bold text-sm h-24"></textarea>
          </div>
        </section>
        ...
        <section class="bg-white/80 p-8 rounded-[2rem] shadow-xl border-2 border-white/50">
          ...
          <div class="space-y-3">
            <label :class="['flex items-center p-5 rounded-2xl cursor-pointer border-2 transition-all', paymentMethod === 'cod' ? 'border-[#658a22] bg-[#f4f7ee]' : 'border-slate-100 bg-white/50']">
            ...
            <label :class="['flex items-center p-5 rounded-2xl cursor-pointer border-2 transition-all', paymentMethod === 'bank_transfer' ? 'border-[#658a22] bg-[#f4f7ee]' : 'border-slate-100 bg-white/50']">
          ...
          <button @click="handleSubmit" :disabled="cartItems.length === 0" class="w-full mt-8 bg-[#1e293b] hover:bg-black text-white p-5 rounded-2xl font-black text-lg uppercase tracking-widest transition-all shadow-lg active:scale-95 disabled:opacity-50">
