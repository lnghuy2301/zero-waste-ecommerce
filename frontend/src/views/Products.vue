...
watch([searchQuery, sortOption], () => { currentPage.value = 1 })
...
<template>
  <div>
      ...
      <div class="p-6 md:p-8">
        <div class="flex items-center justify-between mb-8">
          <h3 class="font-black text-2xl text-slate-800 flex items-center gap-3 uppercase tracking-tight italic">
            <span class="w-10 h-10 bg-[#eef4e6] text-[#658a22] rounded-full flex items-center justify-center">
        ...
        <ul class="space-y-2 relative">
          ...
          <li>
            ...
            >
              <div v-if="route.query.category" class="absolute inset-0 bg-[#eef4e6] transform -translate-x-full group-hover:translate-x-0 transition-transform duration-300 ease-out -z-10"></div>
          ...
          <li v-for="cat in categories" :key="cat.id">
            ...
            >
              <div v-if="route.query.category != cat.id" class="absolute inset-0 bg-[#eef4e6] transform -translate-x-full group-hover:translate-x-0 transition-transform duration-300 ease-out -z-10"></div>
              <span class="relative z-10">{{ cat.name }}</span>
              <span v-if="route.query.category == cat.id" class="material-symbols-outlined text-[18px] relative z-10">check_circle</span>
            </RouterLink>
          </li>
        </ul>
      </div>
    ...
    <main class="max-w-[1400px] mx-auto px-4 md:px-8 py-12 font-sans selection:bg-[#eef4e6] selection:text-[#658a22]">

      <div class="w-full">

        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-10 border-b border-slate-100 pb-6">
          <div>
            <h1 class="text-4xl font-black text-slate-900 uppercase italic tracking-tight">Sản Phẩm <span class="text-[#658a22]">Xanh</span></h1>
            <p class="text-slate-500 mt-2 font-medium text-sm tracking-wide">
              Hiển thị <strong class="text-slate-800">{{ filteredAndSortedProducts.length }}</strong> sản phẩm thân thiện
            </p>
          </div>
          ...
          <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
            ...
            <div class="relative w-full sm:w-48 lg:w-56">
              <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-[#658a22] text-[18px]">sort</span>
              <select
                v-model="sortOption"
        ...
        <div v-if="loading" class="flex flex-col items-center justify-center py-20 text-slate-400 bg-white rounded-3xl border border-dashed border-slate-200">
          <span class="material-symbols-outlined text-4xl text-[#658a22] animate-bounce mb-3">eco</span>
          <p class="font-bold text-sm uppercase tracking-widest">Đang tải...</p>
        </div>

        <div v-else-if="paginatedProducts.length === 0" class="text-center py-24 bg-white rounded-3xl border border-dashed border-slate-200 text-slate-500">
        ...
        <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 lg:gap-7">
          <div
          ...
          >
            <div class="aspect-square relative overflow-hidden rounded-2xl bg-slate-50 mb-4 cursor-pointer" @click="router.push(`/product/${product.id}`)">
              <img
            ...
            <div class="flex flex-col flex-grow px-1">
              <RouterLink :to="`/product/${product.id}`" class="font-black text-slate-800 text-sm mb-2 line-clamp-2 leading-snug group-hover:text-[#658a22] transition-colors" :title="product.name">
                {{ product.name }}
              </RouterLink>
              ...
              <div class="mt-auto pt-3 flex flex-wrap items-center justify-between border-t border-slate-100/80 gap-y-2">
                <div class="flex flex-col">
                  <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Giá từ</span>
                  <div class="flex flex-wrap items-baseline gap-1.5">
                    <span v-if="selectedVariants.has(product.id) && selectedVariants.get(product.id).promotionId" class="text-[11px] text-slate-300 line-through font-bold hidden sm:inline-block">
