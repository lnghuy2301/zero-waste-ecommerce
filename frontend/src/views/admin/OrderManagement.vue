...
const filteredOrders = computed(() => {
  if (!searchAccountId.value.trim()) return orders.value
...
const parseShippingAddress = (addressStr?: string) => {
  ...
  if (parts.length >= 3) {
    return {
      name: parts[0].trim(),
      phone: parts[1].trim(),
      detail: parts.slice(2).join(' - ').trim() // Gom phần còn lại làm địa chỉ phòng khi địa chỉ có chứa " - "
    }
  }
...
<template>
  <div class="max-w-[1400px] mx-auto p-8 min-h-screen">
      ...
      <div class="flex items-center gap-4 mt-3 sm:mt-0">
        <div class="relative">
          ...
          <input
            ...
            class="pl-11 pr-5 py-3.5 bg-white border-2 border-slate-100 focus:border-[#658a22] rounded-2xl outline-none transition-all w-72 font-semibold text-slate-700 text-base shadow-sm"
          />
        </div>
        ...
        <select
          ...
          class="py-3.5 px-4 bg-white border-2 border-slate-100 rounded-2xl outline-none font-semibold text-slate-600 text-base cursor-pointer focus:border-[#658a22] transition-all"
        >
    ...
    <div v-if="loading" class="text-center py-32">
      <div class="inline-block w-12 h-12 border-4 border-[#658a22]/30 border-t-[#658a22] rounded-full animate-spin mb-4"></div>
      <p class="font-bold text-slate-400 uppercase tracking-widest text-sm">Đang tải đơn hàng...</p>
    </div>

    <div
      v-else
      class="bg-white rounded-[28px] shadow-lg overflow-hidden border border-slate-100"
    >
      <div class="overflow-x-auto">
        <table class="w-full text-base min-w-[1100px]">
          ...
          <tr>
            ...
            <th class="px-6 py-5 text-left font-bold text-slate-400 uppercase text-xs tracking-widest">Mã đơn</th>
            <th class="px-6 py-5 text-left font-bold text-slate-400 uppercase text-xs tracking-widest">KH ID</th>
            <th class="px-6 py-5 text-left font-bold text-slate-400 uppercase text-xs tracking-widest">Tổng tiền</th>
            <th class="px-6 py-5 text-left font-bold text-slate-400 uppercase text-xs tracking-widest">Trạng thái</th>
            <th class="px-6 py-5 text-left font-bold text-slate-400 uppercase text-xs tracking-widest">Địa chỉ giao</th>
            <th class="px-6 py-5 text-left font-bold text-slate-400 uppercase text-xs tracking-widest">Ngày tạo</th>
            <th class="px-6 py-5 text-center font-bold text-slate-400 uppercase text-xs tracking-widest">Thao tác</th>
          </tr>
          </thead>
          ...
          <tr
            ...
            :class="selectedOrderIds.includes(order.id) ? 'bg-[#f5fae8] border-l-4 border-[#658a22]' : ''"
          >
            ...
            <td class="px-6 py-5">
              <span class="bg-slate-100 text-slate-600 font-bold px-3 py-1 rounded-lg text-sm">#{{ order.accountId }}</span>
            </td>
            ...
            <td class="px-6 py-5">
              <span
                ...
                :class="{
                  'bg-amber-50 text-amber-700 border border-amber-200': order.status === 'PENDING',
                  'bg-blue-50 text-blue-700 border border-blue-200': order.status === 'PAID',
                  'bg-violet-50 text-violet-700 border border-violet-200': order.status === 'SHIPPING',
                  'bg-emerald-50 text-emerald-700 border border-emerald-200': order.status === 'COMPLETED',
            ...
            <td class="px-6 py-5" @click.stop>
              <div class="flex items-center justify-center gap-2 flex-wrap">
                ...
                <button
                  ...
                  class="px-4 py-2 bg-[#f0f7e0] text-[#4a6a18] rounded-xl text-sm font-bold hover:bg-[#e4f2cc] transition-all whitespace-nowrap"
                >
                  🖨 Hóa đơn
                </button>
                <button
    ...
    >
      <p class="text-sm text-slate-400 font-semibold">
        ...
        trong tổng số <span class="text-slate-700 font-black">{{ filteredOrders.length }}</span> đơn hàng
      </p>
      ...
      <div class="flex items-center gap-2">
        ...
        <template v-for="p in pageNumbers" :key="p">
          <span v-if="p === '...'" class="w-10 h-10 flex items-center justify-center text-slate-300 font-bold">…</span>
          <button
            v-else
      ...
      <Transition name="modal">
        ...
        >
          <div
            ...
            @click.stop
          >
            <div class="sticky top-0 bg-white rounded-t-[32px] px-10 pt-10 pb-6 border-b border-slate-100 z-10">
              <div class="flex items-start justify-between">
                ...
                <div class="flex items-center gap-3">
                  <button
                    @click="openInvoice(currentOrder); showDetailModal = false"
            ...
            <div class="px-10 py-8">
              <div class="grid grid-cols-2 gap-6 mb-8">
                <div class="bg-emerald-50 rounded-2xl p-5">
                  <p class="text-emerald-600 text-xs font-black uppercase tracking-widest mb-1">Tổng tiền</p>
                  <p class="text-3xl font-black text-emerald-700">
                    {{ Number(currentOrder?.totalAmount || 0).toLocaleString('vi-VN') }}₫
                  </p>
                </div>
                <div class="bg-slate-50 rounded-2xl p-5">
                  <p class="text-slate-400 text-xs font-black uppercase tracking-widest mb-2">Trạng thái</p>
                  <span
                ...
                <div class="bg-slate-50 rounded-2xl p-5">
                  <p class="text-slate-400 text-xs font-black uppercase tracking-widest mb-1">Địa chỉ giao hàng</p>
                  <p class="font-bold text-slate-800 text-base">{{ currentOrder?.shippingAddress }}</p>
                </div>
                <div class="bg-slate-50 rounded-2xl p-5">
                  <p class="text-slate-400 text-xs font-black uppercase tracking-widest mb-1">Ngày tạo</p>
              ...
              <div class="space-y-3">
                ...
                >
                  <div>
                    <p class="font-bold text-slate-800 text-base">{{ item.variant?.name || 'Sản phẩm' }}</p>
                ...
                <div v-if="!currentOrder?.orderItems?.length" class="text-center py-8 text-slate-400 font-semibold">
                  Không có sản phẩm
                </div>
              </div>
            </div>
    ...
    <Teleport to="body">
      <Transition name="modal">
        ...
        >
          <div class="bg-white rounded-[32px] w-full max-w-2xl shadow-2xl max-h-[95vh] overflow-auto" @click.stop>

            <div class="flex items-center justify-between px-8 py-5 border-b border-slate-100 print:hidden">
              <p class="font-black text-slate-600 uppercase text-xs tracking-widest">Xem trước hóa đơn</p>
            ...
            <div id="invoice-content" class="px-12 py-10">
              ...
              <div class="text-center mb-8">
                <div class="inline-block bg-[#658a22] text-white px-6 py-2 rounded-full text-sm font-black uppercase tracking-widest mb-3">
                  EcoStore
                </div>
              ...
              <div class="flex justify-between items-start mb-8 pb-6 border-b-2 border-dashed border-slate-200">
                <div>
                  <p class="text-slate-400 text-xs font-bold uppercase tracking-widest">Mã đơn hàng</p>
                  <p class="font-black text-xl text-slate-900 mt-1">{{ currentOrder?.code || 'N/A' }}</p>
                </div>
                <div class="text-right">
                  <p class="text-slate-400 text-xs font-bold uppercase tracking-widest">Ngày xuất</p>
                  <p class="font-bold text-slate-700 mt-1">{{ new Date().toLocaleDateString('vi-VN') }}</p>
                </div>
              </div>
              ...
              <div class="bg-slate-50/80 rounded-2xl p-6 mb-8 border border-slate-100">
                <div class="flex items-center gap-2 mb-5">
                  ...
                  <p class="text-slate-800 text-sm font-black uppercase tracking-widest">Thông tin giao hàng</p>
                </div>

                <div class="space-y-4">
                  <div class="flex gap-4">
                    <div class="bg-white rounded-xl p-4 flex-1 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-slate-100">
                      <p class="text-xs text-slate-400 font-bold uppercase tracking-widest mb-1.5">Người nhận</p>
                    ...
                    <div class="bg-white rounded-xl p-4 flex-1 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-slate-100">
                      <p class="text-xs text-slate-400 font-bold uppercase tracking-widest mb-1.5">Số điện thoại</p>
                      <p class="font-black text-slate-800 text-base">
                  ...
                  <div class="bg-white rounded-xl p-4 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-slate-100">
                    <p class="text-xs text-slate-400 font-bold uppercase tracking-widest mb-1.5">Địa chỉ chi tiết</p>
                    <p class="font-bold text-slate-800 text-base">
              ...
              <table class="w-full mb-8">
                ...
                <tr class="bg-slate-800 text-white">
                  <th class="px-5 py-3 text-left text-xs font-black uppercase tracking-wider rounded-l-xl">Sản phẩm</th>
                  <th class="px-5 py-3 text-center text-xs font-black uppercase tracking-wider">SL</th>
                  <th class="px-5 py-3 text-right text-xs font-black uppercase tracking-wider rounded-r-xl">Thành tiền</th>
                </tr>
                </thead>
                ...
                <tr
                  ...
                  :key="item.id"
                >
                  <td class="px-5 py-4 font-semibold text-slate-700">{{ item.variant?.name || 'Sản phẩm' }}</td>
              ...
              <div class="border-t-2 border-slate-200 pt-5">
                <div class="flex justify-between items-center">
                  <span class="font-black text-slate-500 uppercase text-sm tracking-widest">Tổng cộng</span>
              ...
              <div class="mt-10 pt-6 border-t border-dashed border-slate-200 text-center">
                <p class="text-slate-400 text-sm font-medium">Hóa đơn này được tạo tự động bởi hệ thống.</p>
...
<style scoped>
/* Hiệu ứng đóng mở Modal (vẫn giữ scoped để không ảnh hưởng trang khác) */
.modal-enter-active,
.modal-leave-active {
...
<style>
/* CSS DÀNH CHO IN ẤN (Bỏ chữ 'scoped' để áp dụng được lên toàn bộ body) */
@media print {
  @page {
    /* Căn chỉnh lề trang in cho đẹp, loại bỏ header/footer thừa của trình duyệt */
    margin: 15mm auto;
    size: A4 portrait;
  }

  /* Ép trình duyệt in các mảng màu nền (nút xanh, bảng nền đen...) */
  * {
  ...
  /* TRIỆT TIÊU HÌNH NỀN CỦA TRANG WEB */
  html, body {
    background: #ffffff !important;
    background-image: none !important;
  }

  /* ẨN TOÀN BỘ GIAO DIỆN APP (Mất luôn cái nền xanh đằng sau) */
  body > *:not(#invoice-print-root) {
    display: none !important;
  }

  /* CHỈ HIỂN THỊ DUY NHẤT KHUNG HÓA ĐƠN NỀN TRẮNG */
  #invoice-print-root {
